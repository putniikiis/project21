...
int input_matrix(int **matrix, int height, int width);
void swap(int *a, int *b);
void bubble_sort(int *arr, int length);
void arr_create(int **matrix, int heigth, int width, int **arr);
...
int input(int ***matrix, int *height, int *width) {
    ...
    if (flag == 1) {
        *matrix = alloc(&flag, *height, *width);
        if (flag == 1) {
            flag = input_matrix(*matrix, *height, *width);
        }
    }
    return flag;
}
...
void arr_create(int **matrix, int heigth, int width, int **inArr) {
    int len = 0;
